...
export interface IHardwareAssetLog extends Document {
...
const hardwareAssetLogSchema = new Schema<IHardwareAssetLog>(
  {
    hardwareAssetId: {
...
hardwareAssetLogSchema.index({ hardwareAssetId: 1, timestamp: -1 });
hardwareAssetLogSchema.index({ performedBy: 1, timestamp: -1 });

export default mongoose.model<IHardwareAssetLog>('HardwareAssetLog', hardwareAssetLogSchema);
